...
class RecipeTest < ActiveSupport::TestCase 
    ...
    test "should not save unless title is filled in" do
        recipe = Recipe.new
        assert !recipe.save # save should fail because there are errors. assert recipe.errors[:title].include?("can't be blank")    
    end

    test "should not save unless ingredients is filled in" do
        recipe = Recipe.new
        assert !recipe.save # save should fail because there are errors. assert recipe.errors[:ingredients].include?("can't be blank")
    end

    test "should not save unless instructions is filled in" do
        recipe = Recipe.new
        assert !recipe.save # save should fail because there are errors. assert recipe.errors[:instructions].include?("can't be blank")
    end
